...
> **模型名称必须与提供商要求完全一致**。例如 DeepSeek 用 `deepseek-v4-pro`，不能写成 `deepseek-pro` 或 `deepseek-v4`。请查阅提供商文档确认正确的模型名。**不要手动修改** `~/.codex/config.toml` 中的 `model` 字段——它由 converter 自动同步。
...
> **自定义 provider 无需手动选择**：向导会自动检测认证方式（先试 bearer，失败则切 api_key_header）。
...
**你编辑** `~/.codex-converter/config.toml`。转换器**自动管理** `~/.codex/config.toml` — 不要手动修改其中 `codex-converter` 写入的 `model`、`model_provider` 等字段。
